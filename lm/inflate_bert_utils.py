...
def inflate_BertEmbeddings(
    ...
    def _inflate_embed(orig_w, new_size):
        return inflate(orig_w.detach().clone(), new_size, dim=1, pattern=pattern)
...
def _snapshot_layer(layer):
    """
    Return a dict of {param_name: (mean, std, min, max)} for every parameter
    in a modBertLayer.  Used for before/after logging in inflate_LEMON_educated.
    """
    snap = {}
    for name, param in layer.named_parameters():
        t = param.detach().float()
        snap[name] = (t.mean().item(), t.std().item(), t.min().item(), t.max().item())
    return snap


def _snapshot_bert_embeddings(emb_layer):
    snap = {}
...
def _snapshot_ln(ln_layer):
    snap = {}
    for name, param in ln_layer.named_parameters():
        t = param.detach().float()
        snap[name] = (t.mean().item(), t.std().item(), t.min().item(), t.max().item())
    return snap


def _snapshot_mlm_head(head_layer):
    snap = {}
    for name, param in head_layer.named_parameters():
        t = param.detach().float()
        snap[name] = (t.mean().item(), t.std().item(), t.min().item(), t.max().item())
    return snap
...
def _print_snap(snap, indent='  '):
    """Print a snapshot dict in a compact aligned table."""
    pass


def _print_diff(before, after, indent='  '):
    """Print per-parameter delta between two snapshots."""
    pass
...
def inflate_LEMON_educated(
    ...
    # ================================================================== #
    _section('EMBEDDINGS')
    snap_before = _snapshot_bert_embeddings(orig_bert.bert.embeddings)
    snap_inf_before = _snapshot_bert_embeddings(inf_bert.bert.embeddings)

    _subsection('BEFORE  [orig embeddings]')
    _print_snap(snap_before)
    _subsection('BEFORE  [inf  embeddings]')
    _print_snap(snap_inf_before)
    ...
    _subsection('AFTER   [inf  embeddings]')
    _print_snap(_snapshot_bert_embeddings(inf_bert.bert.embeddings))

    # ================================================================== #
    # WIDTH-ONLY LAYERS  (no depth expansion)                             #
    # ================================================================== #
    _section('WIDTH-ONLY LAYERS  (orig layers 0..{})'.format(no_inflation_depth - 1))

    for i in range(no_inflation_depth):
        _subsection('Layer {}  orig -> inf[{}]  (width-only)'.format(i, i))

        snap_orig = _snapshot_layer(orig_layers[i])
        snap_inf_b = _snapshot_layer(inf_layers[i])
        print('  [orig layer {}]'.format(i))
        _print_snap(snap_orig)
        print('  [inf  layer {} — before]'.format(i))
        _print_snap(snap_inf_b)

        inflate_modBertLayer(orig_layers[i], inf_layers[i], **_layer_kw)

        snap_inf_a = _snapshot_layer(inf_layers[i])
        print('  [inf  layer {} — after]'.format(i))
        _print_snap(snap_inf_a)
        print('  [inf  layer {} — delta vs before]'.format(i))
        _print_diff(snap_inf_b, snap_inf_a)
    ...
    for i in range(no_inflation_depth, orig_depth):
        ...
        snap_orig = _snapshot_layer(orig_layers[i])
        snap_inf_b = _snapshot_layer(inf_layers[ni])
        print('  [orig layer {}]'.format(i))
        _print_snap(snap_orig)
        print('  [inf  layer {} — before]'.format(ni))
        _print_snap(snap_inf_b)

        inflate_modBertLayer(orig_layers[i], inf_layers[ni], **_layer_kw)

        snap_inf_a = _snapshot_layer(inf_layers[ni])
        print('  [inf  layer {} — after]'.format(ni))
        _print_snap(snap_inf_a)
        print('  [inf  layer {} — delta vs before]'.format(ni))
        _print_diff(snap_inf_b, snap_inf_a)
        ...
        if inflate_new_layers:
            ...
            snap_inf_b = _snapshot_layer(inf_layers[zi])
            print('  [orig layer {}]'.format(i))
            _print_snap(snap_orig)   # reuse orig snap from above
            print('  [inf  layer {} — before]'.format(zi))
            _print_snap(snap_inf_b)
            ...
            snap_inf_a = _snapshot_layer(inf_layers[zi])
            print('  [inf  layer {} — after]'.format(zi))
            _print_snap(snap_inf_a)
            print('  [inf  layer {} — delta vs before]'.format(zi))
            _print_diff(snap_inf_b, snap_inf_a)
    ...
    snap_b = _snapshot_ln(orig_bert.bert.encoder.ln_f)
    snap_inf_b = _snapshot_ln(inf_bert.bert.encoder.ln_f)
    _subsection('BEFORE  [orig ln_f]')
    _print_snap(snap_b)
    _subsection('BEFORE  [inf  ln_f]')
    _print_snap(snap_inf_b)
    ...
    _subsection('AFTER   [inf  ln_f]')
    _print_snap(_snapshot_ln(inf_bert.bert.encoder.ln_f))
    ...
    snap_b = _snapshot_mlm_head(orig_bert.cls.predictions)
    snap_inf_b = _snapshot_mlm_head(inf_bert.cls.predictions)
    _subsection('BEFORE  [orig predictions]')
    _print_snap(snap_b)
    _subsection('BEFORE  [inf  predictions]')
    _print_snap(snap_inf_b)
    ...
    _subsection('AFTER   [inf  predictions]')
    _print_snap(_snapshot_mlm_head(inf_bert.cls.predictions))
